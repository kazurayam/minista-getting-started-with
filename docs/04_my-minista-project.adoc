...
では `$ROOT` の下に `my-minista-project` を作ろう。プロジェクトフォルダの名前はユニークにしなければならないが、その他の手順は `minimal-minista-project` と作るのと同じ。
...
これに整合するように、`my-minista-project/src/assets/css` ディレクトリを作ってその中に `index.css` ファイルを格納しました。との中に格納しました。同じように `my-minista-project/src/assets/images` ディレクトリを作って画像ファイルを格納しました。
